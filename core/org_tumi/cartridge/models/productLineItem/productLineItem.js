'use strict'; var productDecorators = require('*/cartridge/models/product/decorators/index');
...
module.exports = function productLineItem(product, apiProduct, options) {
    ...
    productDecorators.availability(product, options.quantity, apiProduct.minOrderQuantity.value, apiProduct.availabilityModel); productLineItemDecorators.quantity(product, options.quantity);
    ...
    productDecorators.customProductAttributes(product, apiProduct);
    productLineItemDecorators.discountBonusLineItems(product, options.lineItem.UUID);
    productLineItemDecorators.monogram(product, options.lineItem, options.lineItemCntr);
    productDecorators.monogram(product, apiProduct);
    productLineItemDecorators.giftbox(product, options.lineItem);
    productLineItemDecorators.availability(product, options.quantity, apiProduct.minOrderQuantity.value, apiProduct.availabilityModel);
    return product;
};
