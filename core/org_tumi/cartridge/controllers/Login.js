...
server.get(
    ...
    function (req, res, next) {
        ...
        var enableJanrain = Site.getCurrent().getCustomPreferenceValue('enableJanrain');

        res.render('/account/loginModal', {
            navTabValue: navTabValue || 'login',
            rememberMe: rememberMe,
            userName: userName,
            actionUrl: actionUrl,
            ...
            oAuthReentryEndpoint: 1,
            createAccountUrl: createAccountUrl,
            enableJanrain: enableJanrain
        });

        next();
    }
);
server.get('CheckAuthenticatedStatus', function (req, res, next) {
...
server.get('OAuthReentry', server.middleware.https, consentTracking.consent, function (req, res, next) {
    ...
    if (!finalizeOAuthLoginResult) {
        res.redirect(URLUtils.url('Login-Create'));
        return next();
    }
