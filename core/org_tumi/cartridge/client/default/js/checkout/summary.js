...
function updateTotals(totals) {
    ...
    $('.grand-total').text(totals.grandTotal);

    if (totals.totalDiscount.value > 0) {
        $('.order-discount').removeClass('hide-order-discount');
        $('.order-discount-total').text('- ' + totals.totalDiscount.formatted);
    } else {
...
function updateOrderProductSummaryInformation(order) {
    ...
    order.shipping.forEach(function (shipping) {
        shipping.productLineItems.items.forEach(function (lineItem) {
            var pli = $('.checkout-content-right [data-product-line-item=' + lineItem.UUID + ']');
