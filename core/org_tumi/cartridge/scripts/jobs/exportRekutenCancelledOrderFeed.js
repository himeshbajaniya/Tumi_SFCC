...
function getCompleteOrderDetails(args) {
    ...
    var Calendar = require('dw/util/Calendar');
    var query = '(shippingStatus = {0} OR status = {1}) AND lastModified >= {2} AND lastModified <= {3}';
    var fromRange = new Calendar();
    fromRange.add(Calendar.DAY_OF_MONTH, -(args.days_before));
    var ordersIterators = OrderMgr.searchOrders(query, null, Order.SHIPPING_STATUS_SHIPPED, Order.ORDER_STATUS_CANCELLED, fromRange.time, new Date()).asList();
