...
function getPurchaseUnit(currentBasket, isCartFlow) {
    ...
    } else if (!isCartFlow && ShipToMeShipment) {
        purchaseUnit.shipping = createShippingAddress(ShipToMeShipment.getShippingAddress());
...
function getBARestData(isCartFlow) {
    ...
    if (hasOnlyGiftCertificates(currentBasket) || isBasketHavingOnlyStorePickupitems) {
        baTokenData.body.plan.merchant_preferences.skip_shipping_address = true;
    } else if (!isCartFlow) {
        var shippingAddress = cartHelper.getShipToMeShipment(currentBasket).getShippingAddress();
 ...
 */
 function getServiceName(siteID) {

    var ppConstants = require('*/cartridge/scripts/util/paypalConstants');
    var serviceName = ppConstants.SERVICE_NAME;

    if (siteID === 'tumi-ca') {
        serviceName = ppConstants.SERVICE_NAME_CA;
    }
    return serviceName;
 }
