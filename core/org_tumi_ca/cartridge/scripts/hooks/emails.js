    ...
    try {
        ...
        if (!empty(order) && !empty(config)) {
            ...
            */
            ...
            var consignments = emarsysHelper.splitLineItemAsConsignments(order).values().toArray();
            sendData.data = emarsysHelper.populateOrderData(order);
            sendData.data.Order.Consignments = consignments;
