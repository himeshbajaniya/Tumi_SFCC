...
var run = function run() {
    ...
    var serviceClient;
    if (empty(keyAlias)) {
      serviceClient = ServiceMgr.getFTPService(serviceID);
    } else {
      serviceClient = ServiceMgr.getFTPServiceForSSH(serviceID, keyAlias);
    }

    var filePaths = serviceClient.listRemoteFiles(sourceFolder, filePattern, recursive);
    ...
    serviceClient.downloadFiles(filePaths, targetFolder, deleteRemoteFiles, recursive);
