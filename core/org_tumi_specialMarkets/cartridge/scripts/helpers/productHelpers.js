...
function getAllBreadcrumbs(cgid, pid, breadcrumbs) {
    ...
    var category;
    var product;
    const hiddenCategoryId = 'hidden';
    const productCategoryId = 'productcatalog';
    if (pid) {
    ...
    if (category) {
        if (category.ID === hiddenCategoryId) {
            var productCategory = CatalogMgr.getCategory(productCategoryId);
            breadcrumbs.push({
                htmlValue: productCategory.displayName,
                url: URLUtils.url('Search-Show', 'cgid', productCategory.ID)
            });
        } else {
            breadcrumbs.push({
