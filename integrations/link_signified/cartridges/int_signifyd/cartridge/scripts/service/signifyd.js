 ...
 *
 * @param {dw.order.Shipments} shipments to be verifyed
...
function getRecipient(shipments, email) {
    ...
    if (!empty(shipments)) { // eslint-disable-line no-undef
...
function getProducts(products) {
    ...
    for (var i = 0; i < products.length; i += 1) {
        ...
        if (empty(primaryCat) && !product.product.isMaster()) { // eslint-disable-line no-undef
            primaryCat = product.product.masterProduct.getPrimaryCategory();
        }
        var parentCat = !empty(primaryCat) ? primaryCat.getParent() : null; // eslint-disable-line no-undef
        ...
        result.push({
            ...
            itemPrice: product.adjustedPrice.value,
            itemSubCategory: !empty(primaryCat) ? primaryCat.ID : null, // eslint-disable-line no-undef
            itemCategory: !empty(parentCat) ? parentCat.ID : (!empty(primaryCat) ? primaryCat.ID : null), // eslint-disable-line no-undef, no-nested-ternary
...
function getCardBin(mainPaymentInst) {
    var cardBin = null;
    try {
        if (!empty(mainPaymentInst.getCreditCardNumber()) && mainPaymentInst.getCreditCardNumber().indexOf('*') < 0) { // eslint-disable-line no-undef
            cardBin = mainPaymentInst.getCreditCardNumber().substring(0, 6);
        } else if (!empty(session.forms.billing.creditCardFields) // eslint-disable-line no-undef
            && !empty(session.forms.billing.creditCardFields.cardNumber) && !empty(session.forms.billing.creditCardFields.cardNumber.value)
            && session.forms.billing.creditCardFields.cardNumber.value.length > 10) { // eslint-disable-line no-undef
            cardBin = session.forms.billing.creditCardFields.cardNumber.value.substring(0, 6); // eslint-disable-line no-undef
        }
    } catch (e) {
...
function getDiscountCodes(couponLineItems) {
    var discountCodes = [];
    if (!empty(couponLineItems)) { // eslint-disable-line no-undef
        ...
        while (iterator.hasNext()) {
            ...
            while (priceAdjustments.hasNext() && empty(discountAmount) && empty(discountPercentage)) { // eslint-disable-line no-undef
                var priceAdj = priceAdjustments.next();
                var discount = priceAdj.getAppliedDiscount();

                if (discount.getType() === dw.campaign.Discount.TYPE_AMOUNT) { // eslint-disable-line no-undef
                    discountAmount = discount.getAmount();
                } else if (discount.getType() === dw.campaign.Discount.TYPE_PERCENTAGE) { // eslint-disable-line no-undef
...
function getParams(order) {
    var SignifydCreateCasePolicy = dw.system.Site.getCurrent().getCustomPreferenceValue('SignifydCreateCasePolicy').value; // eslint-disable-line no-undef
    var SignifydDecisionRequest = dw.system.Site.getCurrent().getCustomPreferenceValue('SignifydDecisionRequest').value; // eslint-disable-line no-undef
    var SignifydPassiveMode = dw.system.Site.getCurrent().getCustomPreferenceValue('SignifydPassiveMode'); // eslint-disable-line no-undef
    ...
    var paramsObj = {
        ...
        purchase: {
            ...
            currency: dw.system.Site.getCurrent().getDefaultCurrency(), // eslint-disable-line no-undef
                    ...
                    if (result.ok) {
                        ...
                        if (SignifydCreateCasePolicy === 'PRE_AUTH') {
                            ...
                            if (answer.checkpointAction) {
                                ...
                                if (answer.recommendedAction) { // eslint-disable-line no-lonely-if
                                    if (answer.recommendedAction === 'REJECT') {
                                        declined = true;
                                    }
                                } else {
                                    if (answer.decisions.paymentFraud.status) { // eslint-disable-line no-lonely-if
                        ...
                        Transaction.wrap(function () {
                            order.custom.SignifydCaseID = String(caseId); // eslint-disable-line no-param-reassign
                            if (SignifydCreateCasePolicy === 'PRE_AUTH') {
                                var orderUrl = 'https://www.signifyd.com/cases/' + caseId;
                                order.custom.SignifydOrderURL = orderUrl; // eslint-disable-line no-param-reassign

                                if (typeof answer.checkpointAction !== 'undefined') {
                                    order.custom.SignifydFraudScore = answer.score; // eslint-disable-line no-param-reassign
                                    order.custom.SignifydPolicy = answer.checkpointAction; // eslint-disable-line no-param-reassign
                                    order.custom.SignifydPolicyName = answer.checkpointActionReason || ''; // eslint-disable-line no-param-reassign
                                } else {
                                    order.custom.SignifydFraudScore = answer.decisions.paymentFraud.score; // eslint-disable-line no-param-reassign
                                    order.custom.SignifydPolicy = answer.recommendedAction || answer.decisions.paymentFraud.status; // eslint-disable-line no-param-reassign
                                    order.custom.SignifydPolicyName = answer.checkpointActionReasons || ''; // eslint-disable-line no-param-reassign
                                }
                            }
                        });
...
function getproductLineItems(productLineItems) {
    ...
    if (!empty(productLineItems)) { // eslint-disable-line no-undef
    ...
    if (EnableCartridge) {
        if (order && order.currentOrderNo) {
            try {
                ...
                for (var index in shipments) { // eslint-disable-line no-restricted-syntax, guard-for-in
