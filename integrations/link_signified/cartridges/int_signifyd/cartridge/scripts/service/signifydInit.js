...
function createCase() {
    var service = LocalServiceRegistry.createService('Signifyd.REST.CreateCase', {
        ...
        filterLogMessage: function (msg) {
            if (!empty(msg) && dw.system.System.getInstanceType() === dw.system.System.PRODUCTION_SYSTEM) { // eslint-disable-line no-undef
                msg = msg.replace(/\"cardBin\"\:\w*/, '"cardBin":"******"'); /* eslint no-param-reassign:0, no-useless-escape:0 */
...
function cancelGuarantee() {
    var service = LocalServiceRegistry.createService('Signifyd.REST.CancelGuarantee', {
        createRequest: function (svc, args) {
            ...
            svc.addHeader('Authorization', 'Basic ' + authKey);
            var url = svc.getURL();
            url = url.replace(/caseId/g, args.signifydCaseId);
            svc.setURL(url);
            if (args.requestObj) {
                return JSON.stringify(args.requestObj);
            }
            return null;
        },
        parseResponse: function (svc, client) {
