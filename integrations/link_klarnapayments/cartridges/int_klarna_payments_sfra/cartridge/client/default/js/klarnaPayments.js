...
KlarnaCheckout.initPlaceOrderStage = function (defer) {
    ...
    if (this.isKlarnaPaymentCategory(selectedPaymentMethod)) {
        ...
        $('.card-number-wrapper').attr('data-type', 'klarna');
    }
...
KlarnaCheckout.refreshKlarnaPaymentOptions = function () {
    ...
    var $selectedPaymentOptionEl = $('.payment-information .active').closest('li');
...
KlarnaCheckout.getKlarnaPaymentMethod = function (methodId) {
    var klarnaMethodId = (methodId !== undefined) ? methodId.replace(this.prefix + '_', '') : '';
...
KlarnaCheckout.initKlarnaSubmitPaymentButton = function () {
    ...
    $klarnaSubmitPaymentBtn.on('click', function (event) {
        ...
        var $tabContent = $($paymentCategory.find('.active').attr('href'));
        var paymentMethodId = this.getKlarnaPaymentMethod($paymentCategory.attr('data-method-id'));
...
KlarnaCheckout.handlePaymentNeedsPreassesment = function () {
    ...
    $billingAddressForm.on('change', function () {
        var tabContentId = $('.payment-options .active').attr('href');
...
KlarnaCheckout.updatePaymentSummary = function (order) {
    ...
    if (firstPaymentInstrument && firstPaymentInstrument.paymentMethod === 'KLARNA_PAYMENTS') {
        ...
        htmlToAppend += '<div class="method-name">Klarna</div>';
...
KlarnaCheckout.obtainBillingAddressData = function () {
    ...
    if ($billingform.length > 0) {
        address.given_name = $billingform.find('#billingFirstName') ? $billingform.find('#billingFirstName').val() : '';
        address.family_name = $billingform.find('#billingLastName') ? $billingform.find('#billingLastName').val() : '';
        address.street_address = $billingform.find('#billingAddressOne') ? $billingform.find('#billingAddressOne').val() : '';
        address.street_address2 = $billingform.find('#billingAddressTwo') ? $billingform.find('#billingAddressTwo').val() : '';
        address.city = $billingform.find('#billingAddressCity') ? $billingform.find('#billingAddressCity').val() : '';
        address.region = $billingform.find('#billingState') ? $billingform.find('#billingState').val() : '';
        address.postal_code = $billingform.find('#billingZipCode') ? $billingform.find('#billingZipCode').val() : '';
        address.country = $('#billingCountry') ? $('#billingCountry').val() : '';
        address.phone = ($billingform.find('#phoneNumber') && $billingform.find('#phoneNumber') != 'null') ? $billingform.find('#phoneNumber').val() : '';
    }

    if ($('#storePickupOnly') && $('#storePickupOnly').val() === 'true') {
        var $billingContactForm = $('#billing-contact-info');
        address.given_name = $billingContactForm.find('#billingFirstName').val();
        address.family_name = $billingContactForm.find('#billingLastName').val();
        address.phone = $billingContactForm.find('#phoneNumber').val();
        address.email = $billingContactForm.find('#contactEmail').val();
    }
    ...
    if ($('#email').length) {
        address.email = $('#email')[0].value;
    } else {
        address.email = $('#email-guest')[0].value;
    }
...
KlarnaCheckout.obtainShippingAddressData = function (billingAddress) {
    ...
    if ($('#storePickupOnly') && $('#storePickupOnly').val() === 'true') {
        $shippingAddressBlock = $('#bopis-shipping-address');
    } 
    var $contactInfoBlock = $('.contact-info-block');
    var $customerInformationBlock = $('.customer-information-block');
...
KlarnaCheckout.buildShippingAddressData = function ($addressBlock, billingAddress) {
    ...
    if ($('#storePickupOnly') && $('#storePickupOnly').val() === 'true') {
        var $billingContactForm = $('#billing-contact-info');
        var address = {
            given_name: $billingContactForm.find('#billingFirstName').val(),
            family_name: $billingContactForm.find('#billingLastName').val(),
            phone: $billingContactForm.find('#phoneNumber').val(),
            street_address: $addressBlock.find('.ship-to-address1').text() ? $addressBlock.find('.ship-to-address1').text() : '',
            street_address2: $addressBlock.find('.ship-to-address2').text() ? $addressBlock.find('.ship-to-address2').text() : '',
            city: $addressBlock.find('.ship-to-city').text() ? $addressBlock.find('.ship-to-city').text() : '',
            postal_code: $addressBlock.find('.ship-to-zip').text() ? $addressBlock.find('.ship-to-zip').text() : '',
            region: $addressBlock.find('.ship-to-st').text() ? $addressBlock.find('.ship-to-st').text() : '',
            country: $('#billingCountry') ? $('#billingCountry').val() : '',
        };
    } else {
        var address = {
...
KlarnaCheckout.getMultiShipDefaultAddress = function (billingAddress) {
    var $multishippingForms = $('.multi-shipping');
    var address = this.buildShippingAddressData($multishippingForms.first(), billingAddress);
...
KlarnaCheckout.getSelectPaymentMethodElement = function () {
    var $selectPaymentMethodInner = $('.payment-options .active');
...
KlarnaCheckout.getSelectedPaymentMethod = function (methodId) {
    ...
    if (typeof paymentMethodId !== 'undefined') {
        klarnaMethodId = paymentMethodId.replace(this.prefix + '_', '');
    }
