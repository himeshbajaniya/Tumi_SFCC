...
var kpVCNDecrypt = require('*/cartridge/scripts/VCN/klarnaVCNDecryption');
...
function callCreditCardAuthorizationHook(order) {
    ...
    var hook = 'app.payment.processor.' + CREDIT_CARD_PROCESSOR_ID.toLowerCase();
    ...
    processorResult = HookMgr.callHook('app.payment.processor.' + CREDIT_CARD_PROCESSOR_ID.toLowerCase(), 'Authorize', order.orderNo, paymentInstrument, paymentProcessor);
...
function handle(basket) {
    ...
    var amount = giftCardHelpers.calculateNonGiftCertificateAmount( basket );
    ...
    if (!kpVCNEnabledPreferenceValue) {
        ...
        try {
            ...
            if (!isSettlementCreated) {
                ...
                var vcnDecryptionResult = {};
                vcnDecryptionResult = kpVCNDecrypt.handleVCNDecryption(order);
                saveVCNData(order, vcnDecryptionResult);
...
function saveVCNData(order, vcnDecryptionResult) {

    var paymentInstrument = order.getPaymentInstruments('KLARNA_PAYMENTS')[0];
    var card;
    try {
        if (vcnDecryptionResult.cardNumberVCN && vcnDecryptionResult.cardExpiryMonthVCN && vcnDecryptionResult.cardExpiryYearVCN && vcnDecryptionResult.cardCvvVCN && vcnDecryptionResult.cardTypeVCN) {
            card = session.forms.billing.creditCardFields;
            Transaction.wrap(function () {
                card.cardType.value = vcnDecryptionResult.cardTypeVCN;
                card.cardNumber.value = vcnDecryptionResult.cardNumberVCN;
                paymentInstrument.setCreditCardType(vcnDecryptionResult.cardTypeVCN);
                paymentInstrument.setCreditCardNumber(vcnDecryptionResult.cardNumberVCN);

                card.expirationMonth.value = Number(vcnDecryptionResult.cardExpiryMonthVCN);
                card.expirationYear.value = Number(vcnDecryptionResult.cardExpiryYearVCN);
                card.securityCode.value = vcnDecryptionResult.cardCvvVCN;
                paymentInstrument.setCreditCardExpirationMonth(Number(vcnDecryptionResult.cardExpiryMonthVCN));
                paymentInstrument.setCreditCardExpirationYear(Number(vcnDecryptionResult.cardExpiryYearVCN));
            });
        ...
        Logger.error('saveVCNData: ' + e.message);
    }
    return { error: true };
}
/**
