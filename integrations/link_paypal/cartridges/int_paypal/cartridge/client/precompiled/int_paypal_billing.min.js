...
if (window.paypal && !isBAEnabled) {
  ...
  var ppPaymentOptionID = document.querySelector('.nav-item .paypal-tab').parentElement.getAttribute('data-method-id');
  var activeTab = document.querySelector('.payment-information');
  if (activeTab.getAttribute('data-payment-method-id') === 'PayPal') {
      document.querySelector('.nav-item .paypal-tab').click();
      $('.payment-information').data('payment-method-id', ppPaymentOptionID);
  }
...
function onApprove(data, actions) {
  ...
  Object(_api__WEBPACK_IMPORTED_MODULE_0__["getOrderDetailsCall"])(data.orderID).then(function (orderData) {
      ...
      $('input[name="dwfrm_billing_addressFields_address1"]').val(orderData.payer.address.address_line_1);
      $('input[name="dwfrm_billing_addressFields_city"]').val(orderData.payer.address.admin_area_2);
      $('input[name="dwfrm_billing_addressFields_postalCode"]').val(orderData.payer.address.postal_code);
      $('select[name$=_billing_addressFields_states_stateCode]').val(orderData.payer.address.admin_area_1);
...
function assignEmailForSavedBA() {
  ...
  var $selectedAccount = $restPaypalAccountsList.querySelector('option:checked');
  var paypalRadioVal = $('.paypal-tab').hasClass('active');
  ...
  if (Object(_billingHelper__WEBPACK_IMPORTED_MODULE_0__["isNewAccountSelected"])($restPaypalAccountsList)) {
    ...
    if(paypalRadioVal) {
      $('.continue-billing-btn').removeClass('d-none');
    }
  }
}
/**
...
function onApprove(_ref) {
  ...
  return Object(_api__WEBPACK_IMPORTED_MODULE_0__["createBillingAgreementCall"])(billingToken).then(function (_ref2) {
    ...
    var allfields = contactInfoFields.concat(billingAddressFields);
