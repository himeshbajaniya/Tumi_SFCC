...
addressHelper.updateOrderShippingAddress = function (basket, shippingInfo) {
    ...
    Transaction.wrap(function () {
        ...
        shipping.setPhone(shippingInfo.phone ? shippingInfo.phone.phone_number.national_number : '');
...
addressHelper.updateBAShippingAddress = function (basket, shippingAddress) {
    ...
    if (!recipient_name && shippingAddress && shippingAddress.shipping && shippingAddress.shipping.name && shippingAddress.shipping.name.full_name) {
    	recipient_name = shippingAddress.shipping.name.full_name;
    }

    if (phone && phone.phone_number && phone.phone_number.national_number) {
        phone = phone.phone_number.national_number;
    }

    if (shippingAddress && shippingAddress.shipping && shippingAddress.shipping.address) {
        country_code = shippingAddress.shipping.address.country_code;
        city = shippingAddress.shipping.address.admin_area_2;
        line1 = shippingAddress.shipping.address.address_line_1;
        postal_code = shippingAddress.shipping.address.postal_code;
        state = shippingAddress.shipping.address.admin_area_1;
    }
    ...
    Transaction.wrap(function () {
        ...
        var shipping = shipToMeShipment.getShippingAddress() || shipToMeShipment.createShippingAddress();
