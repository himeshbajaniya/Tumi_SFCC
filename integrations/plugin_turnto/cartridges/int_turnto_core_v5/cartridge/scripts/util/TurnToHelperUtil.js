...
var TurnToHelper = {
     ...
     * @returns {Hashmap} - Return map of TurnTo keys with locales, authKey
     */
    getHashMapOfKeys: function () {
        ...
        var rg = new RegExp('\\s+', 'gm'); // eslint-disable-line prefer-regex-literals
    ...
    getProductSku: function (lookupId) {
        ...
        // var product = ProductMgr.getProduct(lookupId);
        // if (product.master) {
        //     product = product.variationModel.defaultVariant;
        // }
        var productSku = useVariants ? lookupId : TurnToHelper.getParentSku(lookupId);
        // var productSku = useVariants ? product : TurnToHelper.getParentSku(lookupId);
        // Returning Master ID for products that has variation(color/size) otherwise Returns Variation ID
        return productSku;
    }
};
