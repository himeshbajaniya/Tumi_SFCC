...
function process(product) {
    ...
    try {
        // Non-localized data
        // IMAGEURL
        var image = product.getImage('main', 0);
        var imageURL = '';
        if (image == null) {
            image = product.getImage('alt', 0);
        }
        if (image == null) {
        ...
        for (var j = 0; j < bundledProducts.size(); j += 1) {
        ...
        keyValues.forEach(function (key) {
            ...
            localesArray.forEach(function (l) {
                request.setLocale(l);
                var url = getItemUrl(URLUtils.http('Product-Show', 'pid', product.getID()).toString());
                var item = {
            ...
            var localejson = {
                ...
                itemurl: getItemUrl(URLUtils.http('Product-Show', 'pid', product.getID()).toString()),
