...
server.get('MoreList', function (req, res, next) {
    ...
    if (publicView && req.querystring.id) {
        ...
        list = productListHelper.getList(req.currentCustomer.raw, { type: 12 });
    }
    var WishlistModel = require('*/cartridge/models/productList');
    var wishlistModel = new WishlistModel(
        list,
...
server.get('RemoveProduct', function (req, res, next) {
    var list = productListHelper.removeItem(req.currentCustomer.raw, req.querystring.pid, { req: req, type: 12 });
...
server.post('EditProductListItem', function (req, res, next) {
    ...
    var productList = productListHelper.getList(req.currentCustomer.raw, { type: 12 });
    ...
    if (newItemExist) {
        if (newItemExist.UUID !== requestUuid) {
            productListHelper.removeItem(req.currentCustomer.raw, previousProductId, { req: req, type: 12 });
        }
    } else {
        try {
