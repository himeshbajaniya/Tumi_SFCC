...
if (IsCartridgeEnabled) {
    server.append('SubmitPayment', server.middleware.https, csrfProtection.validateAjaxRequest, function (req, res, next) {
        ...
        if (paymentForm.paymentMethod.value == 'KLARNA_PAYMENTS') {
            return next();
        }
        ...
        if (Object.keys(creditCardErrors).length || Object.keys(billingFormErrors).length) {
            ...
            viewData.paymentInformation = {
                ...
                expirationMonth: {
                    value: parseInt(
                        paymentForm.creditCardFields.expirationMonth.value,
                        10
                    ),
            ...
            this.on('route:BeforeComplete', function (req, res) { // eslint-disable-line no-shadow
                ...
                Transaction.wrap(function () {
                    ...
                    if (billingData.storedPaymentUUID) {
                        if (req.currentCustomer.profile.phone) {
                            billingAddress.setPhone(req.currentCustomer.profile.phone);
                        } else if (billingData.phone) {
    ...
    if (!empty(currentBasket)) {
        ...
        if (paymentIntruments.length > 0) {
            for (var index in paymentIntruments) {    //eslint-disable-line
                if (paymentIntruments[index].creditCardType) creditCardType = paymentIntruments[index].creditCardType
            }
        }
    } else {
