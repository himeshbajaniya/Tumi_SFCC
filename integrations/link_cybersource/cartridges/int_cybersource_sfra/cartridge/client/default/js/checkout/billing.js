        ...
        if (order.billing.payment &&
            ...
            order.billing.payment.selectedPaymentInstruments.forEach((selectedPaymentInstrument) => {
                if (selectedPaymentInstrument.paymentMethod === 'CREDIT_CARD' &&
                    ...
                    document.getElementById('submit-order').className = 'btn btn-primary btn-block submit-order ' + selectedPaymentInstrument.paymentMethod.toLowerCase();
    ...
    if (order.billing.payment && order.billing.payment.selectedPaymentInstruments && order.billing.payment.selectedPaymentInstruments.length > 0) {
        order.billing.payment.selectedPaymentInstruments.forEach((selectedPaymentInstrument) => {
            ...
            if (isCSType) {
                ...
                order.billing.payment.selectedPaymentInstruments.length > 0 && selectedPaymentInstrument.paymentMethod === 'KLARNA_PAYMENTS') {
                htmlToAppend += '<div class="payment">';
                htmlToAppend += '<div class="method-name">' + selectedPaymentInstrument.name + '</div>';
                htmlToAppend += '<div class="category-name">' + selectedPaymentInstrument.categoryName + '</div>';
                if (typeof selectedPaymentInstrument.amountFormatted !== 'undefined') {
                    htmlToAppend += '<div class="amount">' + selectedPaymentInstrument.amountFormatted + '</span>';
                }
                htmlToAppend += '</div>';
            } else if (order.billing.payment && order.billing.payment.selectedPaymentInstruments &&
                order.billing.payment.selectedPaymentInstruments.length > 0) {
