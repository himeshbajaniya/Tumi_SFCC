...
function SecureAcceptanceAuthorize(orderNumber, pi, pmntProcessor) {
    ...
    var additionalArgs = {};
    // eslint-disable-next-line
    var saveCard = ('saveCard' in session.forms.billing.creditCardFields) ? session.forms.billing.creditCardFields.saveCard.value : '';
    if (saveCard) {
...
exports.Authorize = function (orderNumber, paymentInstrument, paymentProcessor) {
    ...
    if ((paymentMethod.equals(CybersourceConstants.METHOD_CREDIT_CARD)
            && (CsSAType == null || CsSAType.equals(CybersourceConstants.METHOD_SA_FLEX))) || paymentMethod.equals(CybersourceConstants.METHOD_VISA_CHECKOUT) || paymentMethod.equals(CybersourceConstants.METHOD_GooglePay) || paymentMethod.equals('KLARNA_PAYMENTS')) {
        var SecureAcceptanceHelper = require(CybersourceConstants.SECUREACCEPTANCEHELPER);
        return SecureAcceptanceHelper.AuthorizeCreditCard({ PaymentInstrument: pi, Order: order });
    }
