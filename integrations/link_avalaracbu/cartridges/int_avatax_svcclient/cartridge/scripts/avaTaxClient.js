...
function initservice() {
    ...
    if (session.privacy.sitesource && session.privacy.sitesource === 'sgjc') {
        ...
        clientHeaderStr = 'SF B2C QuickStart || 20.1.0v2; a0o0b000005cGx5AAE'; // (app name); (app version); (library name); (library version); (machine name)
    }
...
function refundTransaction(companyCode, transactionCode, refundTransactionModel) {
    initservice();
    ...
    svc.setURL(url);
    // service call
    var httpResult = svc.call(refundTransactionModel);
    return responseFilter(httpResult);
}
