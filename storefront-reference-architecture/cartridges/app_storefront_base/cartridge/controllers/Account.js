 ...
 * @param {httpparameter} - loginRememberMe - Whether or not the customer has decided to utilize the remember my email feature.
...
server.post('PasswordResetDialogForm', server.middleware.https, function (req, res, next) {
    ...
    if (email) {
        ...
        if (isValid) {
            ...
            var isEmarsysEnable = require('dw/system/Site').getCurrent().getCustomPreferenceValue('emarsysEnabled');
            if (isEmarsysEnable !== true && resettingCustomer) {
